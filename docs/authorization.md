...
Вы можете сгенерировать политику, используя команду `make:policy` Artisan. Сгенерированная политика будет помещена в каталог `app/Policies`. Если этот каталог не существует в вашем приложении, то Laravel предварительно создаст его:

    php artisan make:policy PostPolicy

Команда `make:policy` сгенерирует пустой класс политики. Если вы хотите создать класс с заглушками методов политики, связанных с просмотром, созданием, обновлением и удалением ресурса, то вы можете указать параметр `--model` при выполнении команды:
