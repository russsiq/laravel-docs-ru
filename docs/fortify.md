...
Вся логика визуализации шаблона `register` Fortify может быть определена с помощью метода `registerView` класса `Laravel\Fortify\Fortify`. Обычно этот метод следует вызывать из метода `boot` поставщика `App\Providers\FortifyServiceProvider` вашего приложения:
...
Вся логика визуализации шаблона `forgot-password` Fortify может быть определена с помощью метода `requestPasswordResetLinkView` класса `Laravel\Fortify\Fortify`. Обычно этот метод следует вызывать из метода `boot` поставщика `App\Providers\FortifyServiceProvider` вашего приложения:
...
Вся логика визуализации шаблона `reset-password` Fortify может быть определена с помощью метода `resetPasswordView` класса `Laravel\Fortify\Fortify`. Обычно этот метод следует вызывать из метода `boot` поставщика `App\Providers\FortifyServiceProvider` вашего приложения:
...
Вся логика визуализации шаблона `verify-email` Fortify может быть определена с помощью метода `verifyEmailView` класса `Laravel\Fortify\Fortify`. Обычно этот метод следует вызывать из метода `boot` поставщика `App\Providers\FortifyServiceProvider` вашего приложения:
...
Вся логика визуализации шаблона `confirm-password` Fortify может быть определена с помощью метода `confirmPasswordView` класса `Laravel\Fortify\Fortify`. Обычно этот метод следует вызывать из метода `boot` поставщика `App\Providers\FortifyServiceProvider` вашего приложения:
