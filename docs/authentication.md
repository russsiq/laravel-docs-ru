...
Вы можете определить своих собственных охранников аутентификации, используя метод `extend` фасада `Auth`. Вы должны разместить ваш вызов метода `extend` внутри [поставщика служб](providers.md). Поскольку Laravel уже содержит `AuthServiceProvider`, мы можем разместить код в этом поставщике:
