...
Вы можете использовать команду `make:migration` [Artisan](artisan.md) для создания миграции базы данных. Новая миграция будет помещена в ваш каталог `database/migrations`. Каждое имя файла миграции содержит временную метку, которая позволяет Laravel определять порядок миграций:
