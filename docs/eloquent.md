...
- [Создание классов модели](#generating-model-classes)
...
Для начала создадим модель Eloquent. Модели обычно находятся в каталоге `app\Models` и расширяют класс `Illuminate\Database\Eloquent\Model`. Вы можете использовать команду `make:model` [консоли Artisan](artisan.md) для создания новой модели:
...
Помимо методов, предоставляемых базовым классом коллекции Laravel, класс коллекции Eloquent предоставляет [несколько дополнительных методов](eloquent-collections.md#available-methods), которые специально предназначены для взаимодействия с коллекциями моделей Eloquent.
...
Подобно методу `chunk`, метод `cursor` может использоваться для значительного уменьшения потребления памяти вашим приложением при итерации десятков тысяч записей модели Eloquent.

Метод `cursor` выполнит только один запрос к БД; однако отдельные модели Eloquent не будут включены в результирующий набор, пока они не будут фактически итерированы. Следовательно, только одна модель Eloquent хранится в памяти в любой момент времени при итерации с использованием курсора. Внутри метод `cursor` использует [генераторы PHP](https://www.php.net/manual/en/language.generators.overview.php) для реализации этого функционала:
...
Eloquent предоставляет методы `isDirty`, `isClean` и `wasChanged` для проверки внутреннего состояния модели и определения того, как изменились ее атрибуты с момента первоначального извлечения модели.
...
В приведенном ниже примере, если существует рейс с пунктом отправления «Окленд» и пунктом назначения «Сан-Диего», его столбцы «цена» и «со скидкой» будут обновлены. Если такой рейс не существует, будет создан новый рейс, который будет иметь атрибуты, полученные в результате слияния первого массива аргументов со вторым массивом аргументов:
...
Метод `restore` также может использоваться при построении запросов, использующих [отношения](eloquent-relationships.md):
...
Однако, поскольку это может быть громоздким, Laravel предоставляет метод `orWhere` «более высокого порядка», который позволяет вам свободно связывать диапазоны без использования замыканий:
...
По желанию можно определить, являются ли две модели «одинаковыми». Метод `is` используется для быстрой проверки того, что две модели имеют одинаковый первичный ключ, таблицу и соединение с БД:

    if ($post->is($anotherPost)) {
        //
    }

Метод `is` также доступен при использовании [отношений](eloquent-relationships.md) `belongsTo`, `hasOne`, `morphTo`, и `morphOne`. Этот метод особенно полезен, если вы хотите сравнить связанную модель без запроса на получение этой модели:
...
Событие `retrieved` сработает, когда существующая модель будет извлечена из БД. Когда новая модель сохраняется в первый раз, инициируются события `creating` и `created`. События `update` / `updated` будут выполняться при изменении существующей модели и вызове метода `save`. События `saving` / `saved` будут выполняться при создании или обновлении модели.
...
При необходимости вы можете использовать [последовательных анонимных слушателей событий](events.md#queuable-anonymous-event-listeners) при регистрации событий модели. Это проинструктирует Laravel выполнить слушателя событий модели в фоновом режиме, используя [очередь](queues.md) вашего приложения:
...
Если прослушивается множество событий в модели, то можно использовать наблюдателей, чтобы сгруппировать пользовательских слушателей в одном классе. Классы наблюдателей имеют имена методов, созвучные событиям Eloquent, которые необходимо прослушивать. Каждый из этих методов получает затронутую модель в качестве единственного аргумента. Команда Artisan `make:observer` — это самый простой способ создать новый класс наблюдателя:
