...
Теперь, когда пользователи вашего пакета выполнят команду `vendor:publish`, ваши веб-активы будут скопированы в указанное место публикации. Поскольку каждый раз при обновлении пакета обычно требуется перезаписывать веб-активы, то может использоваться флаг `--force`:
