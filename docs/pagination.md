...
- Требуется, чтобы порядок был основан как минимум на одном уникальном столбце или уникальной комбинации столбцов.
- Требуется, чтобы направления `order by` (по убыванию / по возрастанию) были одинаковыми, если имеется несколько выражений `order by`.
