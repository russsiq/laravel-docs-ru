...
> {note} Если вы выполняете команду `config:cache` в процессе развертывания, то вы должны быть уверены, что вызываете функцию `env()` только из ваших файлов конфигурации. После кеширования конфигурации, ваш файл `.env` не будет загружен, следовательно, все вызовы функции `env()` вернут `null`.
